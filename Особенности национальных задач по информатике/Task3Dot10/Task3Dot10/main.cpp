...
void printAnswer(int current, int finish, bool start)
{
  if(current != finish || start)
  {
    printAnswer(previous[finish][current], finish, false);
  }
  printf("%d ", current + 1);
}
...
int main()
{
  ...
  for(int T = 0; T < QWE; T++)
  {
    ...
    for(int i = 0; i < m; i++)
    {
      ...
      lists[from - 1].push_back(make_pair(to - 1, make_pair(c, b)));
    }
    double zDown = 0;
    double zTop = 100000;
    double z;
