...
void Solve()
{
  //BEGIN Решая квадратное уравнение, находим время столкновения
  double a, b, c;
  ...
  //END Решая квадратное уравнение, находим время столкновения

  double realConnectionTime = min(max(0.0, connectionTime.first), max(0.0, connectionTime.second));
  //Если уравнение не имеет решения или встреча произойдёт после времени t
  ...
  point2d centralVector = (c2.pos + c2.speed * realConnectionTime)
    - (c1.pos + c1.speed * realConnectionTime);
  double centralVectorLen = centralVector.getLen();

  double v1 = c1.speed.getLen();
  double v1OnCentralVector = v1 != 0 ? v1 * getCos(centralVector, c1.speed) : 0.0;
  point2d yComponent1 = {-centralVector.y, centralVector.x};
  if(cross(centralVector, yComponent1) * cross(centralVector, c1.speed) < 0)
  {
    yComponent1 *= -1.0;
  }
  double ySpeed1 = sqrt(abs(pow2(v1) - pow2(v1OnCentralVector)));
  yComponent1 *= ySpeed1 / centralVectorLen;

  double v2 = c2.speed.getLen();
  double v2OnCentralVector = v2 != 0 ? v2 * getCos(centralVector, c2.speed) : 0.0;
  point2d yComponent2 = {-centralVector.y, centralVector.x};
  if(cross(centralVector, yComponent2) * cross(centralVector, c2.speed) < 0)
  {
    yComponent2 *= -1.0;
  }
  double ySpeed2 = sqrt(abs(pow2(v2) - pow2(v2OnCentralVector)));
  yComponent2 *= ySpeed2 / centralVectorLen;
  
  double q1 = c1.m * v1OnCentralVector + c2.m * v2OnCentralVector;
  double q2 = c1.m * pow2(v1OnCentralVector) + c2.m * pow2(v2OnCentralVector);
  a = c1.m * c2.m + pow2(c2.m);
  b = -2.0 * c2.m * q1;
  c = pow2(q1) - c1.m * q2;
  pair<double, double> v2Speeds = solveQuadraticEquation(a, b, c);
  double newV2Speed = abs(v2Speeds.first - v2OnCentralVector) > abs(v2Speeds.second - v2OnCentralVector)
    ? v2Speeds.first : v2Speeds.second;
  double newV1Speed = (q1 - c2.m * newV2Speed) / c1.m;
  ...
  {
    ac1.speed = centralVector.getCopyWithLen(newV1Speed) + yComponent1;
    ac2.speed = centralVector.getCopyWithLen(newV2Speed) + yComponent2;
  }

  ac1.pos += ac1.speed * otherTime;
  ac2.pos += ac2.speed * otherTime;
}
